...
class List extends Component {
  ...
  loadOutlet() {
    ...
    axios.get(urlShringTxt, clientheaderconfig).then((res) => {
      if (res.data.status === "ok") {
        var result = [{ label: "Common", value: "Common" }];
  ...
  loadList(offset) {
    var addParams = "";
    if (Object.keys(this.state.selected_outlet).length > 0) {
      addParams += "&outlet_id=" + this.state.selected_outlet.value;
    }
                          ...
                          this.state.listdata.map((item, index) => {
                            return (
                              <tr key={index}>
                                ...
                                <td>
                                  {item.delivery_time_slot_type === "1"
                                    ? item.delivery_time_setting_interval_time
                                    : "N/A"}
                                </td>
                                <td>{item.delivery_time_setting_cutt_off}</td>
                                <td>
                                  {item.delivery_time_slot_type === "1"
                                    ? "Time"
                                    : "Time Slot"}
                                ...
                                <td>
                                  {showStatus(
                                    item.delivery_time_setting_status
                                  )}
                                </td>
                                <td>
